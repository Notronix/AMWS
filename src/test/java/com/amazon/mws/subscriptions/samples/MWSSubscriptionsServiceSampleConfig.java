...
public class MWSSubscriptionsServiceSampleConfig
{
    ...
    public static synchronized MWSSubscriptionsServiceAsyncClient getAsyncClient()
    {
        if (client == null)
        {
            MWSSubscriptionsServiceConfig config = new MWSSubscriptionsServiceConfig().withServiceURL(serviceURL);
            client = new MWSSubscriptionsServiceAsyncClient(accessKey, secretKey, appName, appVersion, config, null);
        }
